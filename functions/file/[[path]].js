...
export async function onRequest(context) {  // Contents of context object
    ...
    // 检查是否通过分享链接访问(带 share token 的请求绕过白名单限制)
    const shareToken = url.searchParams.get('share');
    const isSharedAccess = shareToken ? await validateShareToken(env, shareToken, fileId) : false;

    // 检查引用域名是否被允许(分享链接访问跳过此检查)
    if (!isSharedAccess && !isDomainAllowed(context)) {
...
// 验证分享链接 token 是否有效
async function validateShareToken(env, shareId, fileId) {
    try {
        const db = getDatabase(env);
        ...
        // 检查下载次数限制
        if (share.downloadLimit && share.downloadCount >= share.downloadLimit) return false;

        // 检查 fileId 是否匹配
        if (share.fileId !== fileId) return false;
