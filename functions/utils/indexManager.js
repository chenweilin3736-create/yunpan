...
export async function readIndex(context, options = {}) {
    try {
        ...
        // 文件大小过滤（minSize/maxSize 单位为字节）
        if (minSize !== null || maxSize !== null) {
            filteredFiles = filteredFiles.filter(file => {
                // 优先使用 FileSizeBytes，回退到 FileSize（MB 字符串）
                let sizeBytes = 0;
                if (file.metadata.FileSizeBytes) {
                    sizeBytes = parseInt(file.metadata.FileSizeBytes, 10) || 0;
                } else if (file.metadata.FileSize) {
                    sizeBytes = Math.round(parseFloat(file.metadata.FileSize) * 1024 * 1024) || 0;
                }
        ...
        // 日期范围过滤（startDate/endDate 为毫秒时间戳）
        if (startDate !== null || endDate !== null) {
            filteredFiles = filteredFiles.filter(file => {
                const fileTime = parseInt(file.metadata.TimeStamp, 10) || 0;
                if (startDate !== null && fileTime < startDate) return false;
                if (endDate !== null && fileTime > endDate) return false;
                return true;
            });
        }
        ...
        if (sortBy) {
            filteredFiles.sort((a, b) => {
                ...
                switch (sortBy) {
                    case 'name':
                        valA = (a.metadata.FileName || a.id || '').toLowerCase();
                        valB = (b.metadata.FileName || b.id || '').toLowerCase();
                        break;
                    case 'size':
                        valA = parseInt(a.metadata.FileSizeBytes, 10) || (parseFloat(a.metadata.FileSize) * 1024 * 1024) || 0;
                        valB = parseInt(b.metadata.FileSizeBytes, 10) || (parseFloat(b.metadata.FileSize) * 1024 * 1024) || 0;
                        break;
                    case 'time':
                        valA = parseInt(a.metadata.TimeStamp, 10) || 0;
                        valB = parseInt(b.metadata.TimeStamp, 10) || 0;
                        break;
                    default:
                        return 0;
                }
