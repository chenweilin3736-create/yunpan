...
export async function fetchOthersConfig(env) {
    try {
        ...
        // 网盘模式逻辑：开启时自动关闭图床特有功能
        if (settings.netdiskMode?.enabled) {
            if (settings.randomImageAPI) {
                settings.randomImageAPI.enabled = false;
            }
            if (settings.publicBrowse) {
                settings.publicBrowse.enabled = false;
            }
        }
