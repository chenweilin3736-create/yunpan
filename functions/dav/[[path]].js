...
export async function onRequest(context) {
    ...
    switch (modifiedRequest.method) {
        ...
        case 'COPY': return handleCopy(modifiedRequest, env, context);
        case 'MKCOL': return handleMkcol(modifiedRequest, env, context);
...
function handleOptions(request) {
    return new Response(null, {
        status: 200,
        headers: {
            'Allow': 'OPTIONS, GET, PUT, DELETE, PROPFIND, MOVE, COPY, MKCOL',
            'DAV': '1',
            'MS-Author-Via': 'DAV',
        },
    });
...
// WebDAV COPY 方法：复制文件或目录（不删除源）
async function handleCopy(request, env, context) {
    const destinationHeader = request.headers.get('Destination');
    if (!destinationHeader) {
        return new Response('Destination header is required', { status: 400 });
    }
    ...
    const requestUrl = new URL(request.url);
    if (destinationUrl.host !== requestUrl.host) {
        return new Response('Cannot copy resource to a different server', { status: 502 });
    }
    ...
    try {
        ...
        if (isFolder) {
            // 递归复制目录：列出源目录所有文件，逐个下载并重新上传到目标路径
            const listUrl = new URL(`/api/manage/list`, request.url);
            listUrl.searchParams.set('dir', lookupSource);
            listUrl.searchParams.set('count', -1);
            listUrl.searchParams.set('recursive', 'true');
            ...
            const filesToCopy = listData.files || [];
            for (const file of filesToCopy) {
                const relativePath = file.name.substring(lookupSource.length);
                const newFileId = lookupDest + relativePath;

                // 下载源文件
                const fileDownloadUrl = new URL(`/file/${encodeURIComponent(file.name)}`, request.url);
                const downloadResponse = await fetch(fileDownloadUrl.toString());
                if (!downloadResponse.ok) {
                    return new Response(`Failed to download source file ${file.name}`, { status: 500 });
                }
                ...
                const lastSlashIndex = newFileId.lastIndexOf('/');
                const uploadFolder = lastSlashIndex > -1 ? newFileId.substring(0, lastSlashIndex) : '';
                if (uploadFolder) {
                    uploadUrl.searchParams.set('uploadFolder', uploadFolder);
                }
                ...
                if (!uploadResponse.ok || !Array.isArray(uploadResult) || uploadResult.length === 0 || !uploadResult[0].src) {
                    const errorMsg = uploadResult.error || JSON.stringify(uploadResult);
                    return new Response(`Failed to copy file ${file.name}: ${errorMsg}`, { status: 500 });
                }
            }
            ...
            // 单文件复制：下载源文件 → 上传到目标路径
            const fileDownloadUrl = new URL(`/file/${encodeURIComponent(lookupSource)}`, request.url);
            const downloadResponse = await fetch(fileDownloadUrl.toString());
            if (!downloadResponse.ok) {
                return new Response('Source file not found', { status: downloadResponse.status });
            }

            const fileBlob = await downloadResponse.blob();
            const fileName = lookupSource.split('/').pop();
            const formData = new FormData();
            formData.append('file', fileBlob, fileName);

            const uploadUrl = new URL(`/upload`, request.url);
            uploadUrl.searchParams.set('uploadNameType', 'origin');
            const lastSlashIndex = lookupDest.lastIndexOf('/');
            const uploadFolder = lastSlashIndex > -1 ? lookupDest.substring(0, lastSlashIndex) : '';
            if (uploadFolder) {
                uploadUrl.searchParams.set('uploadFolder', uploadFolder);
            }
            ...
            if (uploadResponse.ok && Array.isArray(uploadResult) && uploadResult.length > 0 && uploadResult[0].src) {
                ...
                const errorMsg = uploadResult.error || JSON.stringify(uploadResult);
                return new Response(`Copy failed: ${errorMsg}`, { status: 500 });
            }
        }
    } catch (error) {
        console.error('COPY operation failed:', error.stack);
        return new Response(`Internal server error: ${error.message}`, { status: 500 });
    }
}

// WebDAV MKCOL 方法：创建空目录（通过上传占位文件实现，因目录为虚拟）
async function handleMkcol(request, env, context) {
    const fullPath = decodeURIComponent(new URL(request.url).pathname.substring(1));
    ...
    // 确保路径以 / 结尾表示目录
    const dirPath = fullPath.endsWith('/') ? fullPath.slice(0, -1) : fullPath;

    try {
        const db = getDatabase(env);

        // 检查目录是否已存在
        const listResponse = await db.list({ prefix: dirPath + '/', limit: 1 });
        if (listResponse.keys && listResponse.keys.length > 0) {
            return new Response('Method Not Allowed', { status: 405 });
        }

        // 通过上传一个 .keep 占位文件来创建目录
        const placeholderContent = new Blob([''], { type: 'application/octet-stream' });
        const formData = new FormData();
        formData.append('file', placeholderContent, '.keep');

        const uploadUrl = new URL(`/upload`, request.url);
        uploadUrl.searchParams.set('uploadNameType', 'origin');
        uploadUrl.searchParams.set('uploadFolder', dirPath);
        ...
        if (uploadResponse.ok && Array.isArray(uploadResult) && uploadResult.length > 0 && uploadResult[0].src) {
            ...
            const errorMsg = uploadResult.error || JSON.stringify(uploadResult);
            console.error('MKCOL: Failed to create placeholder file:', errorMsg);
            return new Response(`Failed to create directory: ${errorMsg}`, { status: 500 });
        }
    } catch (error) {
        console.error('MKCOL operation failed:', error.stack);
        return new Response(`Internal server error: ${error.message}`, { status: 500 });
    }
}
